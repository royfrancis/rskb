...
**Overview of single-cell multiomic technologies**

![Numerous single-cell multiomics sequences have been developed and can be classified into seven types. The blue box represents dual omics technology, and the magenta box represents technology that handles three or more omics simultaneously.](assets/lim-2004-1.webp)

A reference list of the different single-cell library prep methods: https://teichlab.github.io/scg_lib_structs/.

![The structure of molecules in genomic libraries. Sequencing libraries are constructed by combining Atomic Regions to form an adapter-insert-adapter construct. The seqspec for the assay annotates the construct with Regions and meta Regions. @booeshaghi2024machine](assets/booeshaghi-1.webp)

A machine readable schema for genomic assays: https://github.com/pachterlab/seqspec @booeshaghi2024machine
...
- Background noise reduces marker gene specificity by causing genes to appear in incorrect cell types.
- Most contamination comes from ambient RNA, while barcode swapping contributes only a small fraction (≤10%).
- Empty-droplet profiles better represent contamination than endogenous cell-expression profiles and are preferable for modeling background noise.
- CellBender performed best overall, accurately removing contamination while preserving biological variation.
- SoupX underestimated cell-to-cell variation.
- DecontX sometimes over-corrected and removed real biological signal.
- Background correction mainly improves marker gene detection; effects on cell classification and overall data structure are relatively modest.

> Existing methods for background removal are good at removing such stray marker gene molecule counts. In contrast, classification and clustering of cells is rather robust even at high levels of background noise. Consequently, background removal improves the classification of only few cells. Moreover, it seems that for low and moderate background levels the tightening of existing broad structures may go at the cost of fine structure. In summary, for marker gene analysis, we would always recommend background removal, but for classification, clustering and pseudotime analyses, we would only recommend background removal when background noise levels are high.
...
Review @cheng2023review @almet2021landscape @armingol2021deciphering
Comparison @dimitrov2022comparison
